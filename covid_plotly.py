...
text_input = html.Div([
    dcc.Input(id='input-1-state', type='text', value='Input number'),
    dcc.Input(id='input-2-state', type='text', value='Input Location'),
...
form = html.Div(dbc.Form([email_input, country_code_input, phone_input, submit_button]), style={'padding':'1.25rem'})
...
def update_output(n_clicks, input1, input2, input3):
    if n_clicks > 0:
        save_to_csv(input1, input2, input3)
        time.sleep(1)
        return u'''The Button has been pressed {} times, Email is "{}", Code is "{}", and Number is "{}"'''.format(n_clicks, input1, input2, input3)
    else:
        return None    
